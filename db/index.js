...
const CONNECTION_STRING =
  process.env.DATABASE_URL || "postgres://postgres/phenomena-dev";
...
async function getOpenReports() {
  try {
    ...
    const { rows: reports } =
      await client.query(` SELECT * FROM reports WHERE "isOpen" "true"; 
    `);
    // then load the comments only for those reports, using a
    // WHERE "reportId" IN () clause
    const { row: comments } = await client.query(`
...
async function closeReport(reportId, password) {
  try {
    ...
    if (!report.isOpen) {
      throw Error("report are already closed");
    }
    ...
    return { message: "this report is close" };
  } catch (error) {
    throw error;
  }
}
...
async function createReportComment(reportId, commentFields) {
  ...
  try {
    ...
    if (Date.parse(report.expirationDate) < new Date()) {
      throw Error("Sorry time has expired");
    }
